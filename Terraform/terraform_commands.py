...
class TerraformCommand(sublime_plugin.WindowCommand):
    ...
    def run_terraform_command(self, args, working_dir=None, callback=None):
        ...
        thread = threading.Thread(
            ...
            args=(cmd, working_dir, panel, callback)
        )
        thread.start()
...
class TerraformValidateCommand(TerraformCommand):
    ...
    def run(self):
        self.run_terraform_command(["validate", "-json"], callback=self.handle_validate_result)
...
class TerraformFormatCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        try:
            ...
            if formatted_content != self.view.substr(sublime.Region(0, self.view.size())):
    ...
    def is_enabled(self):
        return self.view.file_name() and (
            self.view.file_name().endswith('.tf') or
            self.view.file_name().endswith('.tfvars')
        )
...
class TerraformFormatOnSaveListener(sublime_plugin.EventListener):
    ...
    def should_format(self, view):
        ...
        return (
            view.file_name().endswith('.tf') or
            view.file_name().endswith('.tfvars')
        )
        ...
        try:
            ...
            if items:
                self.window.show_quick_panel(
                    items,
                    lambda idx: self.copy_output(list(outputs.keys())[idx], outputs) if idx >= 0 else None
                )
            else:
