        ...
        if os.path.exists(token_file):
            try:
                with open(token_file, 'r') as f:
                    creds = json.load(f)
                    return creds.get("credentials", {}).get("app.terraform.io", {}).get("token")
...
class TerraformCloudLoginCommand(sublime_plugin.WindowCommand):
    ...
    def prompt_for_token(self):
        ...
        self.window.show_input_panel(
            "Enter Terraform Cloud API Token:",
            "",
            self.on_token_entered,
            None,
            None
        )
...
class TerraformCloudLogoutCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        if sublime.ok_cancel_dialog(
            "Are you sure you want to logout from Terraform Cloud?",
            "Logout"
        ):
            global _cloud_state
            _cloud_state = TerraformCloudState()
